...
from evalueaza_solutie import evaluate_results_task1, evaluate_results_task2
...
def save_predictions_task2(img_predictions, img_filenames):
    ...
    TASK2_PREDICTION_PATH = "./predictii/task2/"
    ...
    for idx in range(len(img_predictions)):
        ...
        for p in img_predictions[idx]:
            ...
            if name in ["bart", "marge", "homer", "lisa"]:
                ...
                print("warn: unexpected branching in save predictions for task 2")
    ...
    for name in ["bart", "homer", "lisa", "marge"]:
        ...
        np.save(f"{TASK2_PREDICTION_PATH}detections_{name}", detections[name])
        np.save(f"{TASK2_PREDICTION_PATH}scores_{name}", scores[name])
        np.save(f"{TASK2_PREDICTION_PATH}file_names_{name}", file_names[name])
...
def eval_task1():
    return evaluate_results_task1("./predictii/task1/", "./validare/task1_gt.txt", 0)
...
def predictions_task1(model: Sequential, imgs):
    ...
    def _get_y_matrix(img):
        ...
        # NOTE: initially, I used some methods that were dividing / multiplying coordinates
        #       the following remarks are no longer relevant, but I kept the implementation
    ...
    for img in imgs:
        ...
        print(f"[TASK 1] prediction for idx {_cnt}")
        _cnt += 1
...
def eval_task2():
    ...
    ap_bart = evaluate_results_task2(solution_path, ground_truth_path, "bart", 0)

    ground_truth_path = ground_truth_path_root + "task2_homer_gt.txt"
    ap_homer = evaluate_results_task2(solution_path, ground_truth_path, "homer", 0)

    ground_truth_path = ground_truth_path_root + "task2_lisa_gt.txt"
    ap_lisa = evaluate_results_task2(solution_path, ground_truth_path, "lisa", 0)

    ground_truth_path = ground_truth_path_root + "task2_marge_gt.txt"
    ap_marge = evaluate_results_task2(solution_path, ground_truth_path, "marge", 0)

    return ap_bart, ap_homer, ap_lisa, ap_marge
...
def predictions_task2(model: Sequential, imgs):
    ...
    for img in imgs:
        ...
        print(f"[TASK 2] prediction for idx {_cnt}")
        _cnt += 1
...
if __name__ == "__main__":
    ...
    print(f"[TASK 1] average precision: {ap}")

    task2_run(True)

    ap_bart, ap_homer, ap_lisa, ap_marge = eval_task2()

    print(f"[TASK 2] average precision (bart, homer, lisa, marge): {ap_bart, ap_homer, ap_lisa, ap_marge}")
    print(f"[TASK 2] mAP: {(ap_bart + ap_marge + ap_lisa + ap_homer) / 4}")
